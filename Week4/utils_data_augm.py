...
def create_optuna_vgg16_data_augmentation(trial):
  tf.keras.backend.clear_session()
  
  rotation_range = trial.suggest_float("rotation_range", -30, 30)
  zoom_range = trial.suggest_float("zoom_range" ,0, 0.5)
  ...
  trial.set_user_attr("parameters", model.count_params())
  history = model.fit(train_generator, batch_size=batch_size, epochs=epochs, validation_data=test_generator, verbose=0)
...
def objective_data_augmentation(trial):
  # Generate our trial model.
  history = create_optuna_vgg16_data_augmentation(trial)
...
def experiment_data_augmentation():
  ...
  study_name = "vgg16-data-augmentation"  # Unique identifier of the study.
  storage_name = "sqlite:///{}.db".format(study_name)
  study = optuna.create_study(study_name=study_name, storage=storage_name, load_if_exists=True, 
                              direction="maximize", sampler=optuna.samplers.TPESampler(), pruner=optuna.pruners.HyperbandPruner())
  study.optimize(objective_data_augmentation, n_trials=n_trials)
  
  outputs_dir = 'outputs/'
  
  fig = plot_contour(study, params=['rotation_range','zoom_range'])
  fig.write_image(outputs_dir+"data_augmentation.jpeg")
